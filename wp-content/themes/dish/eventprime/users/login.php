...
if( ! empty( $args->current_user ) && ! empty( $args->current_user->ID ) ) {?>
    ...
    <!-- Login Form Wrapper -->
    <div class="emagic">
        <div id="ep_attendee_login_form_wrapper" class="<?php echo esc_attr($args->block_login_class) ?>">
            <?php
            $register_text = ep_global_settings_button_title('Register');
             ?>
            <div class="ep-login-form ep-shadow-sm ep-border ep-rounded ep-p-5 ep-mb-3 wp-block-create-block-ep-login-block align<?php echo $args->align?>" style="Color: <?php echo esc_attr($args->textColor)?>; background-color: <?php echo esc_attr($args->backgroundColor)?>;"> 
                <div class="ep-event-avatar ep-bg-primary ep-rounded-circle"><i class="material-icons">&#xE7FF;</i></div> 
                <h4 class="ep-modal-title ep-text-center ep-my-3" style="Color: <?php echo esc_attr($args->textColor)?>;">
