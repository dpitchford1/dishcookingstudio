...
if( ! empty( $args->current_user ) && ! empty( $args->current_user->ID ) ) {?>
  <div class="emagic">
    ...
    <div class="emagic">
        <div id="ep_attendee_register_form_wrapper" class="<?php echo esc_attr($args->block_register_class) ?>"><?php 
            ...
            if ( ! empty( $form ) && $form->form_type == 1 ){
                ...
                <form id="ep_attendee_register_form" class="ep-attendee-register-form ep-shadow-sm ep-border ep-rounded ep-p-5 ep-mb-3 wp-block-create-block-ep-register-block align<?php echo $args->align?>" style="Color: <?php echo esc_attr($args->textColor)?>; background-color: <?php echo esc_attr($args->backgroundColor)?>;" method="post">
