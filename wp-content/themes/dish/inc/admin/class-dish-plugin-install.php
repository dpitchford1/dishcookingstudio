...
if ( ! class_exists( 'Dish_Plugin_Install' ) ) :
	/**
	 * The Dish plugin install class
	 */
	class Dish_Plugin_Install {
		...
		public function plugin_install_scripts( $hook_suffix ) {
			...
			wp_enqueue_script( 'dish-plugin-install', get_template_directory_uri() . '/assets/js/admin/plugin-install' . $suffix . '.js', array( 'jquery', 'updates' ), $dish_version, 'all' );

			wp_enqueue_style( 'dish-plugin-install', get_template_directory_uri() . '/assets/css/admin/plugin-install.css', array(), $dish_version, 'all' );
		}
			...
			if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
				if ( is_plugin_active( $plugin_slug . '/' . $plugin_file ) ) {
					...
					$button = array(
						...
						'url'     => '#',
						'classes' => array( 'dish-button', 'disabled' ),
					);
				...
				</span> <?php echo /* translators: conjunction of two alternative options user can choose (in missing plugin admin notice). Example: "Activate WooCommerce or learn more" */ esc_html__( 'or', 'dish' ); ?>
...
return new Dish_Plugin_Install();
