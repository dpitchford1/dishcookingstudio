...
if ( ! class_exists( 'Dish' ) ) :
	...
	class Dish {
		...
		public function setup() {
			...
			// Loads wp-content/languages/themes/dish-it_IT.mo.
			load_theme_textdomain( 'dish', trailingslashit( WP_LANG_DIR ) . 'themes' );

			// Loads wp-content/themes/child-theme-name/languages/it_IT.mo.
			load_theme_textdomain( 'dish', get_stylesheet_directory() . '/languages' );

			// Loads wp-content/themes/dish/languages/it_IT.mo.
		...
		public function widgets_init() {
			...
			for ( $row = 1; $row <= $rows; $row++ ) {
				for ( $region = 1; $region <= $regions; $region++ ) {
					...
					if ( 1 === $rows ) {
						...
						$footer_region_name = sprintf( __( 'Footer Column %1$d', 'dish' ), $region );

						/* translators: 1: column number */
						$footer_region_description = sprintf( __( 'Widgets added here will appear in column %1$d of the footer.', 'dish' ), $region );
					} else {
						/* translators: 1: row number, 2: column number */
						$footer_region_name = sprintf( __( 'Footer Row %1$d - Column %2$d', 'dish' ), $row, $region );

						/* translators: 1: column number, 2: row number */
						$footer_region_description = sprintf( __( 'Widgets added here will appear in column %1$d of footer row %2$d.', 'dish' ), $region, $row );
					}
			...
			$sidebar_args = apply_filters( 'dish_sidebar_args', $sidebar_args );
			...
			foreach ( $sidebar_args as $sidebar => $args ) {
				 ...
				 *
				 * 'dish_header_widget_tags'
				 * 'dish_sidebar_widget_tags'
				 *
				 * 'dish_footer_1_widget_tags'
				 * 'dish_footer_2_widget_tags'
				 * 'dish_footer_3_widget_tags'
				 * 'dish_footer_4_widget_tags'
				 */
				$filter_hook = sprintf( 'dish_%s_widget_tags', $sidebar );
		...
		public function scripts() {
			...
			wp_enqueue_script( 'dish-navigation', get_template_directory_uri() . '/assets/js/navigation' . $suffix . '.js', array(), $dish_version, true );

			if ( has_nav_menu( 'handheld' ) ) {
				$dish_l10n = array(
					'expand'   => __( 'Expand child menu', 'dish' ),
					'collapse' => __( 'Collapse child menu', 'dish' ),
				);

				wp_localize_script( 'dish-navigation', 'dishScreenReaderText', $dish_l10n );
			}
		...
		public function body_classes( $classes ) {
			...
			if ( ! is_active_sidebar( 'sidebar-1' ) ) {
				$classes[] = 'dish-full-width-content';
			}
		...
		public function navigation_markup_template() {
			$template  = '<nav id="post-navigation" class="navigation %1$s" role="navigation" aria-label="' . esc_html__( 'Post Navigation', 'dish' ) . '">';
			$template .= '<h2 class="screen-reader-text">%2$s</h2>';
			$template .= '<div class="nav-links">%3$s</div>';
			$template .= '</nav>';

			return apply_filters( 'dish_navigation_markup_template', $template );
		}
		...
		public function print_embed_styles() {
			...
			<style type="text/css">
				.wp-embed {
					...
					background-color: <?php echo esc_html( dish_adjust_color_brightness( $background_color, -7 ) ); ?> !important;
				}
